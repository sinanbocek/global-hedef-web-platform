...
import {
   ...
   Landmark, AlertCircle, Monitor, Globe, Mail, Phone, MapPin, CreditCard, AlertTriangle, Loader2, Database, Handshake, ArrowRight, Lock
} from 'lucide-react';
...
export const Settings: React.FC<SettingsProps> = ({ onNavigate }) => {
   const [activeTab, setActiveTab] = useState<'companies' | 'banks' | 'users' | 'brand' | 'partners' | 'products' | 'data_import'>('companies');
   ...
   const [accountForm, setAccountForm] = useState<Partial<BankAccount>>({
      accountName: '', branchName: '', iban: 'TR', currency: 'TL', accountType: 'Vadesiz Mevduat'
   });
   ...
   const handleDeleteBank = (id: string) => {
      setConfirmModal({
         ...
         type: 'danger',
         title: 'Bankayı Sil?',
         message: 'Bu bankayı ve bağlı tüm hesaplarını silmek istediğinize emin misiniz? Bu işlem geri alınamaz.',
         onConfirm: async () => {
            try {
               ...
               closeConfirmModal();
               showSuccess('Başarılı', 'Banka silindi.');
            } catch (error) { showError('Hata', 'Silme işlemi başarısız.'); closeConfirmModal(); }
         }
      });
   };
      ...
      setConfirmModal({
         ...
         onConfirm: async () => {
            try {
               ...
               closeConfirmModal();
               setIsAccountModalOpen(false);
               showSuccess('Başarılı', 'Hesap silindi.');
            } catch (error) { showError('Hata', 'Hesap silinemedi.'); closeConfirmModal(); }
         }
      });
   };
   ...
   const initiateDeleteUser = (id: string) => {
      setConfirmModal({
         ...
         message: 'Bu kullanıcıyı kalıcı olarak silmek istediğinize emin misiniz? Bu işlem geri alınamaz.',
         onConfirm: async () => {
            try {
               const { error } = await supabase.from('settings_users').delete().eq('id', id);
               if (error) throw error;
   ...
   // (Removed old confirmDeleteUser) 

   const openAddAccountModal = (bankId: string) => {
      setSelectedBankId(bankId);
      setEditingAccountId(null);
      setAccountForm({ accountName: '', branchName: '', iban: 'TR', currency: 'TL', accountType: 'Vadesiz Mevduat' });
      setIbanError(null);
      ...
      <div className="max-w-7xl mx-auto space-y-6 animate-in fade-in duration-500 pb-20">
         ...
         <div className="bg-white dark:bg-slate-800 p-1.5 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 flex overflow-x-auto gap-2">
            {[
               ...
               { id: 'data_import', label: 'Veri Aktarma', icon: Upload },
            ].map(tab => (
               <button
                  key={tab.id}
                  onClick={() => {
                     if (tab.id === 'data_import') {
                                 ...
                                 {bank.accounts.map(acc => (
                                    <div key={acc.id} className="flex justify-between items-center p-3 bg-slate-50 dark:bg-slate-700/30 border border-slate-100 dark:border-slate-700 rounded-lg group/acc hover:border-brand-primary/30 transition-colors">
                                       ...
                                       <div className="text-right">
                                          <div className="font-bold text-sm text-slate-800 dark:text-white">{acc.currency}</div>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
               <div className="bg-white dark:bg-slate-800 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-700">
                  <div className="p-6 text-center">
                     ...
                     <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-2">Erişim Kısıtlı</h3>
                     <p className="text-slate-500 dark:text-slate-400 text-sm mb-6">
                        Veri aktarım modülüne erişmek için lütfen yönetici şifresini giriniz.
                     </p>
                     ...
                     <input
                        ...
                        placeholder="Şifre"
                        className={`w-full px-4 py-3 rounded-xl border ${passwordError ? 'border-red-500 ring-2 ring-red-200' : 'border-slate-200 dark:border-slate-600 focus:border-brand-primary'} bg-slate-50 dark:bg-slate-900 outline-none transition-all text-center text-lg tracking-widest mb-2`}
                        autoFocus
                     />

                     {passwordError && <p className="text-red-500 text-xs text-left mb-4 px-1">⚠️ Hatalı şifre</p>}
                     ...
                     <div className="flex gap-3 mt-6">
                        ...
                        <button
                           ...
                           className="flex-1 btn-primary py-2.5 shadow-lg shadow-brand-primary/20"
                        >
                           Giriş Yap
                        </button>
                     </div>
